...
void bpnn_train_kernel(BPNN *net, float *eo, float *eh)
{
  ...
  //printf("Performing Kernels computation\n");
...
void bpnn_train_cpu(BPNN *net, float *eo, float *eh)
{
  int in, hid, out;
  float out_err, hid_err;
...
double t_start = gettime();
  bpnn_layerforward_cpu(net->input_units, net->hidden_units,net->input_weights, in, hid);
  ...
  bpnn_adjust_weights_cpu(net->hidden_delta, hid, net->input_units, in, net->input_weights, net->input_prev_weights);
double t_end=gettime();
